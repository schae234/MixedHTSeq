...
class GenomicFeature( object ):
    ...
    def get_gff_line( self, with_equal_sign=False ):
        ...
        return "\t".join( str(a) for a in ( chrom, source,
...
def mixed_SAM_alignments_with_buffer( alignments, max_buffer_size=3000000):
    '''
          Sometimes you have mixed single and paired end reads. This will yield
          a single end read if possible, but if the read is paired end, it will wait
          till it finds the mate to emit it.
    '''
    # keep a buffer available until we find mates
    almnt_buffer = {}
    ...
    for almnt in alignments:
        ...
        if not almnt.paired_end or (almnt.paired_end and not almnt.mate_aligned):
            ...
            # Generate the key for the mate pair
            matekey = (
                almnt.read.name,
            ...
            if matekey in almnt_buffer:
                ...
                # Otherwise, put the read in the buffer
                almntkey = (
